...
{description}
    ...
    async def get_llm_responses(
        ...
        # agenerate returns LLMResult.generations: list[list[Generation]], one inner list
        # per input prompt. We passed a single prompt, so take index [0] from the outer list,
        # then [0].text for the first (and only) completion.
...
Criterion 2: General expressions of interest or dissatisfaction from the prospect that do not mention cost, pricing, or relevance to their business size.
...
"""
    ...
    def parse_response(self, response: str) -> Dict[str, str]:
        """
        Parse the LLM response to extract positive and negative criteria.

        Expects the response wrapped in <criteria><positive>...</positive><negative>...</negative></criteria> tags.

        Args:
            response: Raw response from the language model

        Returns:
            Dictionary with 'Positive' and 'Negative' keys, or empty strings if parsing fails.
        """
        positive_criteria = ""
        negative_criteria = ""
        response = response.strip()
        # The AI prompt primes the model with "<criteria>", but it may omit the wrapping
        # tags — add them defensively so the regex always has a valid structure to match.
