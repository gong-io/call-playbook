...
class ClassifierBase(ABC):
    ...
    def get_system_prompt(self) -> str:
        """
        Generate the system prompt for classification.
...
You are a knowledgeable annotator with experience in classifying diverse types of textual inputs.
    ...
    def get_ai_prompt(self) -> str:
        """
        Generate the AI prompt used for response prefilling.
    ...
    def create_inputs(
        ...
        """
        # Response prefilling: by injecting an AIMessage with the opening tag
        # (e.g. "<rationale>"), the model is forced to continue from that point,
        # producing chain-of-thought reasoning before the final label — rather than
        # choosing its own response structure.
        inputs = [
            [
    ...
    async def process_batches(
        ...
        for start in range(0, len_inputs, self.batch_size):
            ...
            while retries < max_retries:
                try:
                    # Cap concurrency at half batch_size to avoid rate-limit cascades
                    responses_batch = await self.llm.abatch(
                        inputs_batch,
                        config={"max_concurrency": max(1, self.batch_size // 2)},
                    )
                    ...
                    if "ThrottlingException" in str(e) and retries < max_retries - 1:
                        ...
                        print(f"Batch processing failed after {retries + 1} retries.")
                        raise RuntimeError(
...
"""
    ...
    def parse_response(self, response: str) -> Dict[str, str]:
        ...
        # The AI prompt primes the response with "<rationale>", but sometimes the model
        # omits or misplaces the opening tag — patch defensively before regex parsing.
        ...
        try:
            ...
            if label_match:
                ...
                if label_value in self.target_names:
                    ...
                    # Fallback: tag content is unrecognized — infer label from
                    # whichever class keyword appears exclusively in the response.
                    if "positive" in response_l and "negative" not in response_l:
                        label = "Positive"
                    elif "negative" in response_l and "positive" not in response_l:
                        label = "Negative"
